// Make work in node.js or the browser
...
(function() {
    ...
    function Maze(map) {
        ...
        function topViewCanvas(canvas) {
            if (canvas == undefined) {
                return self.pTopViewCanvas==undefined ? null : self.pTopViewCanvas;
            }
            self.pTopViewCanvas = canvas;
            if (canvas) topdraw();
            return canvas;
        }
        ...
        function threeDCanvas(canvas) {
            if (canvas == undefined) {
                return self.pThreeDCanvas==undefined ? null : self.pThreeDCanvas;
            }
            self.pThreeDCanvas = canvas;
            if (canvas) {
                initPos();
        ...
        function topdraw(canvas) {
            ...
            var vert = self.vert;
            var horiz = self.horiz;

            ctx.beginPath();

            // Draw horizontal lines
            for (var j=0; j<=self.height; j++) {
                var y = Math.round(j*height/self.height);
                var pendown = false
                var row = horiz[j];
                var lastx = 0;
                for (var i=0; i<self.width; i++) {
                    var x = Math.round((i+1)*width/self.width);
                    if (row[i]) {
                        if (!pendown) {
            ...
            for (i=0; i<=self.width; i++) {
                var x = Math.round(i*width/self.width);
                var pendown = false
                var lasty = 0;
                for (var j=0; j<self.height; j++) {
                    var y = Math.round((j+1)*height/self.height);
                    if (vert[j][i]) {
                        if (!pendown) {
        ...
        function topdrawpos(erase) {
            ...
            var left = 1;
            var top = 1;
            var deltax = width/self.width;
            var deltay = height/self.height;
        ...
        exports.UP = {i:0, j:-1};
        exports.DOWN = {i:0, j:1};
        exports.RIGHT = {i:1, j:0};
        exports.LEFT = {i:-1, j:0};
        ...
        function do3dTouch(event, canvas) {
            ...
            h4 = canvas.height / 4;
            var action = null;
            if (x <= w4) action = turnLeft;
            else if (x >= canvas.width-w4) action = turnRight;
            else if (y < h4) action = goForward;
